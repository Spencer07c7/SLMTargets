...
A SLM contains an array of liquid crystal pixels that resemble a LCD display or camera sensor. The purpose of a reflective _phase only_ SLM is to modify the phase of light in a spatially selective manner. When light strikes the surface of the SLM, it is subsequently reflected. The phase of the reflected light depends on the refractive index of the pixel it passes through. It turns out that the refractive index of each pixel is conveniently modifiable by voltage. Thus the user is empowered with the ability to modify the the phase of the reflected wavefront in a spatially specific manner simply by setting the voltage at each pixel to a desired value. In phase only LCoS SLMs, this is achieved by uploading a phase mask image to the SLM. When phase-modulated light is subsequently passed through the objective lens of a microscope it undergoes a Fourier transform, resulting in the spatially varying distribution of light that we call holograms.  
